...
export default function EditGrupo() {
  ...
  const [larguraa, setLargura] = useState('');
  const [alturaa, setAltura] = useState('');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setTamanho(result.data.grupo_medidas);
        setEnabled(result.data.grupo_privado);
        setCodigo(result.data.grupo_codigo);
        setGrupo_Name(result.data.grupo_name);
        setId(result.data._id);
        console.log(result.data.grupo_medidas);
      ...
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl tracking-tight font-bold text-gray-600">
            ...
            {' > '} Código do Grupo: {codigo.toUpperCase()}
          </h2>
        </div>
      </header>
      <main>
