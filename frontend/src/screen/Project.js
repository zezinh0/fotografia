...
export default function Example() {
  ...
  const [{ loading, error, total, eventos }, dispatch] = useReducer(reducer, {
    loading: false,
    error: '',
    total: 0,
    eventos: [],
    ...
    const fetchData = async () => {
      ...
      try {
        const result = await axios.get(
          `/api/grupos/${user_id}?page=${page}&limit=${limit}`
        );
        console.log(result);
        dispatch({
          type: 'FETCH_SUCCESS',
          payload: [result.data.grupos, result.data.total],
        });
