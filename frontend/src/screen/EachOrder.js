...
export default function EachOrder() {
  const iddd = '62f3141f52a24b903a8116a4';
  const params = useParams();
  const { encomenda_id } = params;
  ...
  console.log(imagens);
  useEffect(() => {
    const fetchData = async (encomenda_id) => {
      try {
        const result = await axios.get(`/api/encomenda/getencomenda/${iddd}`);
        setEncomenda(result.data[0]);
        const result2 = await axios.get(`/api/encomenda/imagens/${iddd}`);
        setImagens(result2.data);
      } catch (error) {
        console.log(error);
      }
    };
    fetchData(encomenda_id);
  }, [encomenda_id]);
  return (
