...
const pages = [
  ...
  { name: 'New Event', href: '#', current: true },
];
const projects = [
  {
    id: 1,
...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case 'FETCH_SUCCESS':
      return { ...state, encomendass: action.payload, loading: false };
    case 'FETCH_FAIL':
...
export default function Example() {
  ...
  const onSubmit = async () => {
    ...
    let send = {
      ...
      grupo_codigo: codigo,
      grupo_privado: true,
      grupo_download: download,
      grupo_download_price: download_price,
      grupo_download_id: Date.now(),
    ...
    console.log(send);
    grupo.append('send2', 'sgadiuygasuydgyuasg');
    grupo.append('send', JSON.stringify(send));
    grupo.append('send33', 'sgadiuygasuydgyuasg');
    console.log('ooooooosoidjoiash');
    try {
      console.log('Entrei dasdasd');
      ...
      */
      ...
      if (files.length > 0) {
        console.log('POPOPOPOPOPOPOPOPOPOPOPOPOPOPOPOPO?POPOPOPOPo');
        var formData = new FormData();
        //formData.append('grupo', res.data.data._id);
        formData.append('grupo', '6350193d681409592ea15c2a');
